...
vll segRes(5*2e5,2e18);

void buildTree(ll ind,ll start,ll end,vll &arr){
    if(start==end){
        segRes[ind]=arr[start];
        return ;
    }
    ll mid=start+(end-start)/2;
    buildTree(2*ind+1,start,mid,arr);
    buildTree(2*ind+2,mid+1,end,arr);
    segRes[ind]=min(segRes[2*ind+1],segRes[2*ind+2]);
}

ll queryTree(ll ind,ll start,ll end,ll fst,ll snd){
    if(start>=fst && end<=snd){
        return segRes[ind];
    }
    if(start>snd || end<fst){
        return 2e18;
    }
    ll mid=start+(end-start)/2;
    ll leftRes=queryTree(2*ind+1,start,mid,fst,snd);
    ll rightRes=queryTree(2*ind+2,mid+1,end,fst,snd);
